	...
	$(document).on('keyup','#searchByarticleName', function(e) {	
		...
		$.ajax({
			type: "POST",
			url: base_url+'/SearchAutoCompletetemp1',
...
$(document).on('click','.modalimgview', function(e) {		
	...
	var csrf_token = $('input[name="_token"]').val();
	$.ajax({
		url: base_url+'/update_preview_count_temp',
		dataType: 'json',
		type: 'post',
...
$(document).on('click','a', function(e) {		
	...
	if(eoption == "dpdf"){
		...
		var csrf_token = $('input[name="_token"]').val();
		$.ajax({
			url: base_url+'/update_dpdf_count_temp',
			dataType: 'json',
			type: 'post',
			contentType: 'application/x-www-form-urlencoded',
	...
	if(eoption == "dedit"){
		...
		var userid = $(this).attr('data-user_id');
		$.ajax({
			url: base_url+'/update_depdf_count_temp',
			dataType: 'json',
			type: 'post',
			contentType: 'application/x-www-form-urlencoded',
			data: {_token:csrf_token,infoid:infoid,user_id:userid},
	...
	if(eoption == "shared"){
		...
		var csrf_token = $('input[name="_token"]').val();
		$.ajax({
			url: base_url+'/update_shared_count_temp',
			dataType: 'json',
			type: 'post',
			contentType: 'application/x-www-form-urlencoded',
			data: {_token:csrf_token,infoid:infoid},
...
$(document).ready(function(){
	...
	if(str.length > 1){
		 if(str[0] == 'slides'){
			...
			var csrf_token = $('input[name="_token"]').val();
			$.ajax({
				url: base_url+'/slides/get_info_from_slug',
				dataType: 'json',
				type: 'post',
				contentType: 'application/x-www-form-urlencoded',
