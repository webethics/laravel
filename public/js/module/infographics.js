		...
		$( "#sortable" ).on( "sortupdate", function( event, ui ) {
			...
			if(categoryIndex.length > 0){
				...
				$.ajax({
					type: "POST",
					dataType: 'json',
					url: base_url+'/infographics/sortlist',
...
$(document).on('click', '.delete_infographic' , function() {

	var infographic_id = $(this).data('id');
	...
	var csrf_token = $('meta[name="csrf-token"]').attr('content');
	 $.ajax({
        type: "POST",
		dataType: 'json',
        url: base_url+'/infographics/delete_infographic/'+infographic_id,
        data: {_token:csrf_token,infographic_id:infographic_id,lang:lang},
        success: function(data) {
			if(data.success){
				notification('Success','infographic deleted Successfully','top-right','success',2000);
				$('.user_row_'+infographic_id).hide();
			}else{
				if(data.message){
...
$(document).ready(function(){
	...
	if($("#langauge").val() == 2){
		$('#arabic_form').show();
		$('#arabic_form').removeClass('col-md-6');
		$('#arabic_form').addClass('col-md-12');
	}
	if($("#langauge").val() == 3){
		$('#arabic_form').show();
		$('#english_form').hide();
...
$(document).on('click','#export_infographics', function(e) {
	 e.preventDefault(); 
    ...
    $.ajax({
        type: "POST",
		//dataType: 'json',
        url: base_url+'/export_infographics',
        data: {
			title:$('#title').val(),
			 lang:$('#lang').val(),
			_token:csrf_token},
        success: function(data) {
			...
			if(data.success == false){
				...
				downloadLink.download = "Infographics.csv";
...
$(document).on('click','.switch_status', function(e) {
	...
	$.ajax({
        type: "POST",
		dataType: 'json',
        url: base_url+'/infographics/enable-disable',
        data: {status:user_status,user_id:user_id,_token:csrf_token,lang:lang},
        success: function(data) {
             // IF TRUE THEN SHOW SUCCESS MESSAGE  
			 if(data.success){
				notification('Success','Infographic has been published.','top-right','success',4000);
				
			}else{
             notification('Error','Infographic has been drafted.','top-right','error',4000);
			}	
...
$(document).on('submit','#searchForm', function(e) {
    var lang  = $('#lang').val();
	if(lang == 'ar'){
		url = base_url+'/listinfographics-arabic'
	}else{
		url = base_url+'/listinfographics'
	}
	e.preventDefault(); 
	$('.search_spinloder').css('display','inline-block');
