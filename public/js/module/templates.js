		...
		$( "#sortable" ).on( "sortupdate", function( event, ui ) {
			...
			if(categoryIndex.length > 0){
				...
				$.ajax({
					type: "POST",
					dataType: 'json',
					url: base_url+'/templates/sortlist',
...
$(document).on('click', '.delete_template' , function() {
	...
	var csrf_token = $('meta[name="csrf-token"]').attr('content');
	 $.ajax({
        type: "POST",
		dataType: 'json',
        url: base_url+'/templates/delete_template/'+template_id,
        data: {_token:csrf_token,template_id:template_id,lang:lang},
        success: function(data) {
			if(data.success){
				notification('Success','template deleted Successfully','top-right','success',2000);
				$('.user_row_'+template_id).hide();
			}else{
				if(data.message){
...
$(document).on('click','#export_templates', function(e) {
	 e.preventDefault(); 
    ...
    $.ajax({
        ...
        success: function(data) {
			...
			if(data.success == false){
				...
				downloadLink.download = "Templates.csv";
...
$(document).on('click','.switch_status', function(e) {
	...
	$.ajax({
        type: "POST",
		dataType: 'json',
        url: base_url+'/templates/enable-disable',
        data: {status:user_status,user_id:user_id,_token:csrf_token,lang:lang},
        success: function(data) {
             // IF TRUE THEN SHOW SUCCESS MESSAGE  
			 if(data.success){
				notification('Success','Template has been published','top-right','success',4000);
				
			}else{
             notification('Error','Template has been drafted.','top-right','error',4000);
			}	
...
$(document).on('submit','#searchForm', function(e) {
	var lang  = $('#lang').val();
	if(lang == 'ar'){
		url = base_url+'/listtemplates-arabic'
	}else{
		url = base_url+'/listtemplates'
	}
