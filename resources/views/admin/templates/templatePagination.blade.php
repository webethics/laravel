	 ...
	 @if(is_object($listTemplates) && !empty($listTemplates) && $listTemplates->count())
	  ...
	  @foreach($listTemplates as $key => $template)
		<tr data-user-id="{{ $template->id }}" class="user_row_{{$template->id}}" >
			...
			<input class="custom-switch-input switch_status" id="switch{{ $template->id }}" type="checkbox" data-language="en"  data-user_id="{{ $template->id }}" {{$selected}}>
			...
			<td>
				<a class="action" href ="{{url('admin/templates/edit')}}/{{$template->id}}/en" title="Edit"><i class="simple-icon-note"></i></a>
				
			<a title="Delete Template"  data-id="{{ $template->id }}" data-confirm_type="complete" data-confirm_message ="Are you sure you want to delete the Template?"  data-left_button_name ="Yes" data-language="en"  data-left_button_id ="delete_template" data-left_button_cls="btn-primary" class="open_confirmBox action deleteTemplate"  href="javascript:void(0)" data-role_id="{{ $template->id }}"><i class="simple-icon-trash"></i></a>
			<input type="hidden" name="per_page" value="{{$number_of_records}}" id="per_page"/>
			<input type="hidden" name="page_language" value="en" id="page_number_{{$template->id}}"/>
