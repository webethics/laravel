...
class CustomersController extends Controller
{
	...
	public function customer_search($request,$pagination)
	{
		...
		if($customers){
			foreach($customers as $key=>$value){
				$Subscription = Subscription::where("user_id",$value->id)->first();
				$downloads 	= 	Download::where("user_id",$value->id)->with('articles','arabic_articles')->count();
				if($Subscription){
					$customers[$key]->subscription = $Subscription->subscription_id;
				}
				if($downloads){
					$customers[$key]->downloads = $downloads;
				}
			}
		}
	...
	public function customer_delete($customer_id)
    {
		...
		if($customer_id){
			...
			if($main_customer){
				SocialGoogleAccount::where('user_id',$customer_id)->delete();
				SocialFacebookAccount::where('user_id',$customer_id)->delete();
